...
from solver import LbmSolverMrt
...
def run_single_case(
    ...
    lbm = LbmSolverMrt(cfg, load_mask_from_png(mask_path, nx, ny))
    lbm.init()
    ...
    with tqdm(total=stop_step, desc=f" > {mask_name}", leave=False) as pbar:
        ...
        while step < stop_step:
            ...
            if (
                gui
                or cfg["simulation"].get("save_png")
                or cfg["simulation"].get("save_npy")
            ):
                # 只有在需要顯示或儲存PNG時才渲染圖像
                ...
                if step % cfg["simulation"]["save_step"] == 0:
                    ...
                    if cfg["simulation"].get("save_npy", False):
                        ...
                        dv_dx = np.roll(vel_np[:, :, 1], -1, axis=0) - np.roll(
                            vel_np[:, :, 1], 1, axis=0
                        )
                        du_dy = np.roll(vel_np[:, :, 0], -1, axis=1) - np.roll(
                            vel_np[:, :, 0], 1, axis=1
                        )
                        ...
                        output_data_np = np.concatenate(
                            [vel_np, rho_expanded, vorticity_expanded], axis=-1
                        )
                    ...
                    if output_data_np is not None or img is not None:
                        save_snapshot(
                            ...
                            output_data_np,
                            img,
                            cfg["simulation"].get("save_npy", False),
                        )
...
def _render_frame(lbm: LbmSolverMrt, inlet_v: float) -> np.ndarray:
    """生成速度場與渦度場的對比圖"""
    ...
    img_v = cm.plasma(np.clip(v_mag / (inlet_v * 3.0), 0, 1))[:, :, :3]
